...
class GeminiHistoryService:
    ...
    def is_running(self) -> bool:
        """Return whether a Gemini history sync is active."""
    ...
    def start(self, config: CrawlConfig) -> None:
        ...
        with self._lifecycle_lock:
            if self.is_running():
                raise RuntimeError("A Gemini history sync is already running.")
            if not self._task_lock.acquire("gemini-history-sync"):
                raise RuntimeError(
