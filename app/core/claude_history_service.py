...
class ClaudeHistoryService:
    ...
    def is_running(self) -> bool:
        """Return whether a Claude history sync is active."""
    ...
    def start(self, config: CrawlConfig) -> None:
        ...
        with self._lifecycle_lock:
            if self.is_running():
                raise RuntimeError("A Claude history sync is already running.")
            if not self._task_lock.acquire("claude-history-sync"):
                raise RuntimeError(
