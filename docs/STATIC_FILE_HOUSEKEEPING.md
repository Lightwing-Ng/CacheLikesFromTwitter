...
user-owned Cache service on port `8666` merely to perform housekeeping. Reproducible caches such as
`__pycache__/`, `*.pyc`, `.pytest_cache/`, and disposable test output still require the exact-byte
and process-ownership checks from the shared contract.
